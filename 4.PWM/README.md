...
[`TPM_Init`](#func1)使用指定的配置初始化TPM。  
[`TPM_GetDefaultConfig`](#func2)获取默认配置信息。  
[`TPM_Deinit`](#func3)禁用TPM计数器并关闭模块时钟。  

### Channel mode operations
[`TPM_SetupPwm`](#func4)为PWM输出设置TPM通道，该功能可以为多个通道设置PWM信号属性。每个通道都有自己的[`tpm_chnl_pwm_signal_param_t`](#struct1)结构，用于指定输出信号的占空比和电平模式。然而，相同的脉宽调制周期和脉宽调制模式适用于所有请求脉宽调制输出的信道。信号占空比以脉宽调制周期的百分比提供。其值应介于0和100之间，其中0为非激活信号（0%占空比）和100为始终激活信号（100%占空比）。当产生组合的脉宽调制信号时，所传递的信道号是指信道对号，例如0是指信道0和1，1是指信道2和3。  
[`TPM_UpdatePwmDutycycle`](#func5)更新特定TPM通道的脉宽调制信号占空比。    
[`TPM_UpdateChnlEdgeLevelSelect`](#func6)更新特定TPM通道的级别选择位。这可用于在更改PWM信号时禁用PWM输出。  
[`TPM_SetupInputCapture`](#func7)为输入捕获设置一个TPM通道，用户可以指定捕获边缘。  
...
## 数据结构
### <span id="struct1">`tpm_chnl_pwm_signal_param_t`</span>
...
### <span id="struct2">`tpm_config_t`</span>
...
## 枚举类型
### <span id="enum1">`tpm_clock_prescale_t`</span>
...
### <span id="enum3">`tpm_pwm_level_select_t`</span>
...
### <span id="enum4">`tpm_pwm_mode_t`</span>
...
### <span id="enum5">`tpm_clock_source_t`</span>
