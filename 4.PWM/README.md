...
函数[`TPM_Init()`](#anchor1)使用指定的配置初始化TPM。  
函数[`TPM_GetDefaultConfig()`](#anchor2)获取默认配置信息。  
函数[`TPM_Deinit()`](#anchor3)禁用TPM计数器并关闭模块时钟。  
函数[`TPM_SetupPwm()`](#anchor4)为PWM输出设置TPM通道，该功能可以为多个通道设置PWM信号属性。每个通道都有自己的[`tpm_chnl_pwm_signal_param_t`](#anchor5)结构，用于指定输出信号的占空比和电平模式。然而，相同的脉宽调制周期和脉宽调制模式适用于所有请求脉宽调制输出的信道。信号占空比以脉宽调制周期的百分比提供。其值应介于0和100之间，其中0为非激活信号（0%占空比）和100为始终激活信号（100%占空比）。当产生组合的脉宽调制信号时，所传递的信道号是指信道对号，例如0是指信道0和1，1是指信道2和3。  
函数[`TPM_UpdatePwmDutycycle()`](#anchor10)更新特定TPM通道的脉宽调制信号占空比。  
函数[`TPM_UpdateChnlEdgeLevelSelect()`](#anchor11)更新特定TPM通道的级别选择位。这可用于在更改PWM信号时禁用PWM输出。  
函数[`TPM_SetupInputCapture()`](#anchor12)为输入捕获设置一个TPM通道，用户可以指定捕获边缘。  
...
`base`：TPM外设基址  
`config`：指向用户配置类型[`tpm_config_t`](#anchor6)指针
注意：此函数应该在TPM驱动器使用开始时使用。  

<span id="anchor2">获取默认配置</span>
...
<span id="anchor3">禁用TPM</span>
	void TPM_Deinit(TPM_Type* base)
	

<span id="anchor4">PWM设置</span>
...
参数名称|类型|说明
-|-|-
chnlNumber|[tpm_chnl_t](#anchor8)|要配置的TPM通道
level|[tpm_pwm_level_select_t](#anchor9)|脉宽调制输出激活电平选择
...
参数名称|类型|说明
-|-|-
prescale|[tpm_clock_prescale_t](#anchor7)|TPM时钟预定值
